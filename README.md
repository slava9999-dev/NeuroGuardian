...
- **Ozon Card Awareness:** Автоматический учёт 5% скидки (оплачивает продавец!)
- **Storage Cost Alerts:** Предупреждения о росте тарифов (x2 после 60 дней, x4 после 90)
...
- **Конкретные рекомендации:** Всегда с цифрами и действиями

### 🔄 **n8n Integration:**

- **Полная визуализация** всех процессов автоматизации
- **Sentinel Monitoring:** Проверка цен каждые 5 минут
...
- 💰 Рассчитывает **полную Unit-экономику** с учётом ВСЕХ затрат
- ⚠️ Предупреждает о **скрытых расходах** (Ozon Card, хранение, возвраты)
- 📊 Показывает **конкретные цифры** убытков и годовой impact
- 🎯 Даёт **actionable рекомендации** для защиты прибыли
...
├─ Комиссия: 240₽ (12%)
├─ Логистика: 46₽
├─ Ozon Card: 40₽ (2% от цены!)
└─ ПРИБЫЛЬ: 474₽ (23.7%)

⚠️ РИСК:
Скидка Ozon Card съедает 40₽ с каждого заказа!
...
### 💰 Полный расчёт Unit Economics

**Учитываем ВСЁ:**

- ✅ Комиссии маркетплейса (по категориям, 2025 тарифы)
- ✅ Логистика (первая/последняя миля)
- ✅ **Ozon Card скидка** (5% за счёт продавца, ~40% заказов)
- ✅ Хранение на складе (с учётом роста тарифов)
- ✅ Возвраты (логистика за ваш счёт)
- ✅ Эквайринг
- ✅ СПП (самовыкупы)
- ✅ Упаковка

**Результат:** Вы видите РЕАЛЬНУЮ прибыль, а не иллюзию.
...
1. **Ozon Card (КРИТИЧНО!):**
   ...
   - Средний убыток: 2% от цены
   - **Пример:** 1000₽ товар → 20₽ убыток → 20,000₽/год на 1000 заказов

2. **Хранение на WB:**
   - Первые 60 дней: базовый тариф
   - 61-90 дней: тариф x2
   - После 90 дней: тариф x4 + риск утилизации
   - **Предупреждение на 45-м дне!**
