    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-md bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xl"
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-t border-slate-200">
              ...
              {TUTORIAL_STEPS.map((_, i) => (
                <button
                  ...
                  className={`
                    ...
                    ${i === currentStep ? 'bg-amber-500 w-4' : 'bg-slate-300'}
                  `}
                />
              ))}
            </div>
            ...
            <button
              onClick={handleNext}
              className="px-4 py-2 rounded-xl font-medium bg-amber-500 text-white hover:bg-amber-400 transition-colors"
            >
              {isLast ? 'Готово ✓' : 'Далее →'}
            </button>
          </div>
